...
function makeBar(label, index) {
    ...
    {
      case 'CDown':
        labelEl.innerText = 'Correct Frog Position';
        break;
      case 'WDown':
        labelEl.innerText = 'Frog Position too High';
        break;     
      case 'CUp':
    ...
    data.forEach(({ className, probability }) => {
        ...
        if (strPercentage == "100%")
        {
          if(className=="CDown")
          {
            ...
            if(AccurateStateTracker.includes("WUp"))
            {
              AccurateStateTracker = [];
            }
            else if(AccurateStateTracker.includes("CUp"))
            {
